...
public:
    ...
    virtual P3D getTargetLimbEEPositionAtTime(const RobotLimb* l, double t) {
        P3D target = P3D() + limbTrajectories[l].evaluate_linear(t);
        double x = target.x;
        double y = target.y;
        double z = target.z;
        P3D r_o = P3D(x,-1,z);
        V3D r_v = V3D(0,1,0);
        P3D hitPoint = target;
        terrain.hitByRay(r_o, r_v, hitPoint);
        return hitPoint;
        //return target;
    ...
    virtual P3D getTargetTrunkPositionAtTime(double t) {
        P3D target = P3D() + bFrameMotionPlan.bFramePosTrajectory.evaluate_linear(t);
        double x = target.x;
        double y = target.y;
        double z = target.z;
        P3D r_o = P3D(x,-1,z);
