...
public:
    ...
    void solve(int nSteps = 10) {
        ...
        for (uint i = 0; i < nSteps; i++) {
            ...
            for(auto EETarget: endEffectorTargets){
                auto p = EETarget.p;
                auto rb = EETarget.rb;
                auto target = EETarget.target;
                //gcrr.estimate_linear_jacobian(p, rb, dpdq);
                gcrr.compute_dpdq(p, rb, dpdq);
                auto world_p =  gcrr.getWorldCoordinates(p,rb);
                Matrix J = dpdq.block(0,6,3,q.size() - 6);
                V3D diff = V3D(world_p,target);
                deltaq = (J.transpose() * J).ldlt().solve( J.transpose()*diff );
                //double gamma = 1;
                //deltaq = gamma*J.transpose()*diff;
